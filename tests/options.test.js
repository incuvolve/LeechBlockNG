...
describe('options.js tests', () => {
    ...
    describe('Export/Import', () => {
        it('should call downloadBlobFile when exportOptions is called', () => {
            window.MAX_SETS = 10;
            window.initForm(1);
            document.getElementById('exportOptions').click();
            expect(window.downloadBlobFile).toHaveBeenCalled();
        });

        it('should show an error when downloadBlobFile throws an error', () => {
            window.MAX_SETS = 10;
            window.initForm(1);
            window.downloadBlobFile.mockImplementation(() => {
                throw new Error('test error');
            });
            document.getElementById('exportOptions').click();
            expect(window.$.fn.dialog).toHaveBeenCalledWith('open');
        });
    });
});
